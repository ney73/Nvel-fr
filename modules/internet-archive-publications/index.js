...
(() => {
  ...
  function usablePublicationFile(file) {
    if (!publicFile(file)) return false;
    const size = Number(file.size || 0);
    if (!Number.isFinite(size) || size <= 0) return false;
    const name = String(file.name || "").toLowerCase();
    const formats = normalizedList(file.format);
    if (formats.some((format) => /lcp|encrypted|protected/.test(format))) return false;
    if (name.endsWith(".epub")) return formats.some((format) => format.includes("epub"));
    if (name.endsWith(".pdf")) return !formats.length || formats.some((format) => format.includes("pdf"));
    return false;
  }
  ...
  function supportsMode(item) {
    return recordFormats(item).some((format) => (
      format === "epub" || format.includes("pdf")
    ) && !/lcp|encrypted|protected/.test(format));
  }
  ...
  function openSearchClause(query) {
    ...
    const formats = '(format:EPUB OR format:PDF OR format:"Text PDF" OR format:"Image Container PDF")';
  ...
  function publicationFiles(record) {
    return (Array.isArray(record.files) ? record.files : [])
      .filter(usablePublicationFile)
      .map((file) => {
        const formatName = String(firstValue(file.format) || "").toLowerCase();
        const name = String(file.name || "");
        const format = name.toLowerCase().endsWith(".epub") ? "epub" : "pdf";
        const rank = format === "pdf"
          ? (formatName.includes("image container") ? 0 : (formatName === "pdf" ? 1 : (formatName.includes("text") ? 2 : 3)))
          : 4;
        return { file, format, rank };
      })
      .sort((left, right) => left.rank - right.rank || String(left.file.name).localeCompare(String(right.file.name)));
  }
  ...
  async function discoveryHome() {
    ...
    return { sections: [{ id: "popular", title: "Popular Internet Archive Publications", items: popular.items }, { id: "latest", title: "Recently added Internet Archive Publications", items: latest.items }] };
  }
